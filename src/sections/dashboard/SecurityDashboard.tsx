...
const securityIncidents = [
  ...
  {
    id: 3,
    type: "Téléchargement non autorisé",
    date: "20/04/2025",
    status: "Alerte envoyée",
    ip: "192.0.2.x",
    region: "Belgique",
  },
  {
    id: 4,
    type: "Partage suspect",
    date: "19/04/2025",
...
const SecurityDashboard: React.FC = () => {
  ...
  // Initialiser les réglementations au montage du composant
  useEffect(() => {
    const initializeRegulations = async () => {
      try {
        const regs = await regulatoryFirewall.init();
        setCurrentRegulations(regs);
        setLoading(false);
      } catch (error) {
  ...
  if (!currentRegulations) {
    return (
      <div className="min-h-screen bg-background flex items-center justify-center">
        Erreur lors du chargement des réglementations.
      </div>
    );
  }
    ...
    <section className="mb-8">
      ...
      {activeAlert && (
        <Alert className="mb-4 border-red-500 bg-red-500/10 animate-pulse">
          ...
          <AlertDescription>
            Tentative de copie de contenu détectée il y a quelques secondes.
            Votre contenu reste protégé.
          </AlertDescription>
        </Alert>
      )}
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="bg-muted/20 p-3 rounded-lg">
                <div className="flex justify-between items-center">
                  ...
                  <span className="text-lg font-bold">
                    {securityStats.contentProtected}
                  </span>
                </div>
              </div>
              <div className="bg-muted/20 p-3 rounded-lg">
            ...
            <div className="space-y-4">
              <Alert className="border-green-500 bg-green-500/10">
                <AlertTitle className="text-green-500">
                  Région détectée: {regulatoryFirewall.currentRegion}
                </AlertTitle>
                <AlertDescription>
                  Votre tableau de bord est conforme aux réglementations
                  locales.
                </AlertDescription>
              </Alert>
              ...
              <div className="bg-muted/20 p-3 rounded-lg">
                ...
                <ul className="space-y-2 text-sm">
                  ...
                  <li className="flex items-center">
                    ...
                    {currentRegulations.dataRetentionDays} jours
                  </li>
                  <li className="flex items-center">
                    <span className="w-4 h-4 bg-green-500 rounded-full mr-2 flex-shrink-0"></span>
                    {currentRegulations.requiresContentLabeling
                      ? "Labellisation du contenu activée"
                      : "Labellisation du contenu désactivée"}
                  </li>
                  <li className="flex items-center">
                    <span className="w-4 h-4 bg-green-500 rounded-full mr-2 flex-shrink-0"></span>
                    {regulatoryFirewall.requiresCookieNotice()
                      ? "Notification de cookies requise"
                      : "Notification de cookies non requise"}
                  </li>
                </ul>
              </div>
            </div>
                  ...
                  <TableCell>
                    <span
                      className={`inline-flex items-center px-2 py-1 rounded-full text-xs ${
                        incident.status === "Bloqué"
                          ? "bg-green-500/20 text-green-500"
                          : incident.status === "Alerte envoyée"
