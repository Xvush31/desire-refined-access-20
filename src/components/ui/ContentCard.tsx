...
const ContentCard = ({ 
  ...
  const getBadge = () => {
    switch (type) {
      case 'premium':
        return <span className="premium-badge absolute top-2 left-2 z-20 px-3 py-1 rounded-full text-xs bg-gradient-to-r from-amber-500 to-amber-300 text-white">Premium</span>;
      case 'vip':
        return <span className="vip-badge absolute top-2 left-2 z-20 px-3 py-1 rounded-full text-xs bg-gradient-to-r from-purple-600 to-indigo-400 text-white">VIP</span>;
      default:
        return null;
    }
  };
  ...
  return (
    <div 
      className={cn(
        "content-card relative overflow-hidden rounded-lg group transition-all duration-300 hover:shadow-lg cursor-pointer",
        sizeClasses[size],
        className
      )}
    ...
    >
      ...
      {/* Type badge (premium/vip) */}
      {type !== 'standard' && getBadge()}
